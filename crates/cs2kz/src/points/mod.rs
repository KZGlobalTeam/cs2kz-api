...
///
/// `times` must be sorted ascending (fastest first).
pub fn recalculate_leaderboard(
    ...
    let leaderboard = LeaderboardData {
        dist_params: params,
        tier,
        leaderboard_size: times.len() as u64,
        top_time: times.first().copied().unwrap_or(0.0),
    };

    let records = times
        .iter()
        .map(|&time| calculate_fraction(time, &leaderboard))
        .collect();

    RecalculatedLeaderboard { leaderboard, records }
}
